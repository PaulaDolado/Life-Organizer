...
- **Hobbies**: alta de hobby, registro de sesiones, sesiones-esta-semana calculadas en el cliente a partir de `recentSessions`
...
- **Hobbies**: el original guarda `goalPerWeek`/`sessionsThisWeek` como contadores manuales en el store local. Nuestro modelo `Hobby` no tiene un campo de meta semanal — esta página muestra las sesiones de esta semana **calculadas** a partir de `recentSessions` (que sí devuelve la API), no un contador aparte. La página de Hobbies tampoco existía en el repo original (estaba en el nav pero sin archivo de ruta) — se construyó desde cero siguiendo el mismo lenguaje visual.
