...
describe("scheduleService", () => {
  ...
  describe("addRow", () => {
    it("la primera fila del usuario queda con order 0", async () => {
      ...
      await scheduleService.addRow(1, "08:00 - 10:00");
    ...
    it("una fila nueva se añade con order = último + 1", async () => {
      ...
      await scheduleService.addRow(1);
  ...
  describe("updateRow", () => {
    it("lanza NotFoundError si la fila no existe", async () => {
      ...
      await expect(scheduleService.updateRow(1, 999, { monday: "Cálculo I" })).rejects.toThrow(NotFoundError);
    });

    it("lanza ForbiddenError si la fila es de otro usuario", async () => {
      prismaMock.scheduleRow.findUnique.mockResolvedValue({ id: 1, userId: 2 });
      await expect(scheduleService.updateRow(1, 1, { monday: "Cálculo I" })).rejects.toThrow(ForbiddenError);
    });

    it("solo actualiza las celdas indicadas", async () => {
      prismaMock.scheduleRow.findUnique.mockResolvedValue({ id: 1, userId: 1 });
      prismaMock.scheduleRow.update.mockImplementation(({ data }) => Promise.resolve({ id: 1, ...data }));

      await scheduleService.updateRow(1, 1, { monday: "Cálculo I", friday: "Física II" });
  ...
  describe("deleteRow", () => {
    it("lanza ForbiddenError si la fila es de otro usuario, sin borrar", async () => {
      prismaMock.scheduleRow.findUnique.mockResolvedValue({ id: 1, userId: 2 });
      await expect(scheduleService.deleteRow(1, 1)).rejects.toThrow(ForbiddenError);
    ...
    it("borra la fila propia", async () => {
      ...
      await scheduleService.deleteRow(1, 1);
  ...
  describe("moveRow", () => {
    ...
    it("mover 'up' la primera fila (sin vecina anterior) no hace nada", async () => {
      prismaMock.scheduleRow.findUnique.mockResolvedValue({ id: 1, userId: 1, order: 0 });
      prismaMock.scheduleRow.findFirst.mockResolvedValue(null);

      await scheduleService.moveRow(1, 1, "up");

      expect(prismaMock.scheduleRow.update).not.toHaveBeenCalled();
    });

    it("lanza ForbiddenError si la fila es de otro usuario", async () => {
      prismaMock.scheduleRow.findUnique.mockResolvedValue({ id: 1, userId: 2, order: 0 });
      await expect(scheduleService.moveRow(1, 1, "down")).rejects.toThrow(ForbiddenError);
    });
  });
});
